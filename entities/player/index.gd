...
func get_animation_direction(direction: Vector2):
	$weapon.look_at(entitie_data.movement.mouse_direction)
	var mouse_direction = entitie_data.movement.mouse_direction.normalized()
...
func _physics_process(delta):
	...
	axis = get_axis()
	entitie_data.movement.mouse_direction = get_global_mouse_position()
...
func _input(event):
	if event.is_action_pressed("attack"):
		...
		if entitie_data.equipment.weapon == "bow":
			rpc("spawn_arrow")
		$AttackDuration.start()
	if event.is_action_pressed("interact") and can_interact and droped_item:
		change_weapon(droped_item)

sync func spawn_arrow():
	var arrow_item = ARROW_PROJECTILE.instance()
	var mouse_direction = entitie_data.movement.mouse_direction if is_network_master() else slave_entitie_data.movement.mouse_direction
	...
	$"/root/game/arena".add_child(arrow_item)
